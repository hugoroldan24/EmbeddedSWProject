...
void SPI_Init(){
  ...
  DDRB |= (1<<DD_MOSI) | (1<<DD_SS_SLAVE) | (1<<DD_CLK); /*Set pins as output, DD_SS is the pin (PD7) that is connected to the SS in the module*/ 
  DDRD |= (1<<DD_SS);
  SPCR |= (1<<MSTR) | (1<<SPE);                               /*We configure the arduino as the master and we habilitate the SPI protocol*/
  SPCR &= ~(1<<CPOL) & ~(1<<CPHA);                         /*We set the Clock Polarity and Phase to 0 */
